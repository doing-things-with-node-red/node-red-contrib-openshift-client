...
module.exports = (RED) => {
  function ocNewApp(n) {
    const node = this;
    const {
      templateFieldType, template, parametersFieldType, parameters,
    } = n;
    ...
    node.on('input', async (msg) => {
      ...
      node.template = getNodeField(node, templateFieldType, template, RED, msg);
      node.parameters = getNodeField(node, parametersFieldType, parameters, RED, msg);
      try {
        const argsEnv = {
        ...
        const { stdout } = await oc.newApp(argsEnv);
